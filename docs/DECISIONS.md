...
- 追补（2026-09-15，prompt v5→v6；门禁仍零改动，Dice 35 不动）：
  1. 删除"改写后仍须保留原选项的结论关键词"——它与放宽目标冲突（把模型拉回词面 paraphrase），
     且架构上 lexical anchor 本就是 warning 而非 gate。改为"允许用不同的自然语言表达技术结论，
     不要求保留关键词，但不得改变语义角色/因果/条件/范围/真假"。代价是换词彻底的选项更容易撞上
     逐槽位漂移下限（Dice<35）而 fallback——属预期行为，先跑真实生成的遥测归因再决定 35 是否调整。
  2. "不交换选项顺序"改写为槽位表述："每个输出槽位必须保持原选项的语义角色，不得互换"——
     硬约束从来都是后者（程序按 slot 映射答案），前者只是它的推论；避免未来程序侧 shuffle 与 prompt 措辞冲突。
  3. "新增技术术语不超过 2 个"改为软约束："不得引入新的解题依赖知识或技术前提；场景背景术语可增，
     但不得成为解题必需条件"——真正要防的是新解题依赖，不是新词个数。
  4. 术语统一：Presentation Variant = **同知识重写（same-knowledge rewrite）** = 不改变 assessment identity 的重写；
     "轻量变体/轻量语义变换"降为历史别名（ADR-036 标题与历史记录不追改，`src/ai/variant.ts` 等当前行为注释已改名）。
  5. Dice 35 阈值本次不动：它是"防完全换成另一个选项"的粗粒度网，不是场景变化限制；调参依据只能是真实生成的
     fallback 归因（option-semantic-drift 占比），不是 prompt 措辞。
- 原则（外部评审原话收录）：**题目可以重新写，知识不能重新定义；选项可以重新表达，答案契约不能重新定义。**
...
- 决策：
  ...
  3. **P0 字面锚点 `stemAnchorMissing`（原 `hasStemAnchor`）由 rejection 降级为 warning**：它是**漂移软信号，不再是语义闸门**——字面锚点只能证明「题干仍与主题相关」，无法证明语义等价（反例：原题「为什么 KV Cache 能降低 prefill 成本？」的合法变体「某服务前缀高度重复却仍重复前向计算，如何降低开销？」零锚点命中）。变体安全的真正兜底是另外两条硬边界：`VARIANT_SYSTEM` 的逐项一一对应约束，以及 `answer`/`explanation` 恒取 canonical（变体改歪也不会判错题）。未命中只记 `warning`（导出常量 `STEM_ANCHOR_WARNING`）+ 日志，不阻断、不回退原题。硬门槛只剩结构不变量与长度泄题。
...
- 决策：
  ...
  - **残留清理与证据面收紧（2026-09-01，第三轮）**：① `ai/variant.ts` 的 `RawVariant` 瘦身为 `{ question?, options? }`——即便模型回吐 `answer`/`explanation`，解析阶段也无处可落（此前虽在 `toGeneratedVariant` 丢弃，但类型仍声明，属可被误用的残留）。② `requiredCoverageMet` 的证据面从「题干+选项」收紧为**只看题干**。理由：轻量变体下选项只是「对原选项的逐项语义改写」，核心术语天然会被带进选项文本（问 positional encoding 时选项里必然出现该词），把选项当证据等于让「题干已漂移、靠选项兜底」的变体过检——**知识点出现在选项里 ≠ 题干在考察它**。自此 `hasStemAnchor` 与 `requiredCoverageMet` 的证据面统一为题干（前者要求任一 anchor 命中，后者要求 required 覆盖 ≈2/3，仍互补）。
