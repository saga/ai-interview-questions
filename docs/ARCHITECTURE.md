...
ai/            LLM 适配层，应用只依赖 LLMProvider 接口（实现仅两套：Chrome / PiAI；
   ...
   variant.ts        变体生成（one-shot 重写题干与选项；complete 由 provider 注入，不感知底层）。
                     ...
                     只来自 `src/data/variants/*`（generator=offline），经 applyVariant 落地声明的测量面。
                      VARIANT_SYSTEM 为稳定契约前缀（v5：任务约束 + 选项改写幅度 + 语义角色对应 + JSON 输出契约，ADR-080），
...
- **Prompt 约束**：`VARIANT_SYSTEM` 为同知识重写版（v6，KV-Cache 友好，ADR-080）：要求同知识重写（允许明显换场景/叙事/槽内表达结构、加非解题性背景；结论可用自然语言重述，不强制保留关键词），明令禁止改动选项数量/真假属性/答案/解析/槽位角色（每个输出槽位保持原语义角色，不得互换；顺序由程序统一处理）；`*-options` 风格对选项做**幅度明显**的改写（换视角/句式/主语，而非仅同义替换），否则 near-dup 门禁会判近重复整条丢弃（见 ADR-072）。红线 6 条：新知识点/新解题前提/答案变化/正误翻转/难度明显变化/全新 distractor。`buildUser` 只注入 `topic/requiredConcepts/question/options`（choice 时），不暴露 `answer/explanation/referenceAnswer/angle/difficulty`，从源头切断“LLM 重新决定答案”的路径。离线 assessment 模式另走 `VARIANT_ASSESSMENT_SYSTEM`（v1）+ `generateAssessmentVariant`：同一 Knowledge（topic/tags/requiredConcepts 不变）+ 不同 reasoning path（换测量角度）+ 强制输出三段式 `reasoningGoal`（先→再→排除）与自声明 `angle`/`cognitiveTask`；用户提示词额外携带 `canonicalAngle/canonicalCognitiveTask/canonicalAssessment`（模型不读原文测量面就换不出新路径），仍不携带 answer/explanation/referenceAnswer（见 ADR-078）。
...
- **抗暗示（anti-cueing）硬失败**：`domain/variant.validateVariant` 在选择题分支对**规范化后**的选项跑 `domain/bias.detectOptionLengthBias`；命中长度泄题（正确项全局最长且存在明显过短干扰项，差距 ≥1.8×）即拒绝，并带机器可读 `code='option-length-bias'`，由 `finalizeQuestion` 回退原题、原因码计入 variant 遥测——**不再重新请求 LLM**（变体边界：省掉最耗时的一次重试）。第五轮前该检查位于 `ai/variant.generateVariant`，随「单一校验入口」内移。
...
- **资产契约（`src/schemas/variant.ts`，ADR-081）**：`VariantKind`（surface / context / surface-options / context-options）、`QuestionVariant`（`id/kind/mode/question/options?/angle?/cognitiveTask?/assessment?/generatedAt/generator/promptVersion/sourceHash` + provenance 可选字段 `batch/model/contentHash/sourceSnapshot`）。`mode` 是业务语义（presentation = 同知识重写 / assessment = 不同 reasoning path），不由 `generator` 推断——四象限：offline+presentation ✅ / offline+assessment ✅ / runtime+presentation ✅ / runtime+assessment ❌（schema 硬拒 presentation + face；历史资产缺 `mode` 时回退为 assessment）。、`VariantPool`（`version/generatedAt/promptVersion/variants: Record<id, QuestionVariant[]>`）。`sourceHash = computeVariantSourceHash(canonical)`（FNV-1a）用于 stale 检测；`sourceSnapshot` 存生成时刻 canonical 入指纹字段快照，供 stale 归因（metadata-only→重算 hash，content→重新生成）；provenance 字段只用于审计，**不参与** assessment identity（见 ADR-078）。
...
- **离线生成器 / 审计**：`npm run question:variants`（vite-node）默认 `--mode assessment`（不同 reasoning path，经 `generateAssessmentVariant` + 三段式 reasoningGoal 门禁 + 与 canonical 实质不同检查；`--mode presentation` 同知识重写表达，恒继承测量面，ADR-080），支持 `--ids/--topics/--count/--kind/--mode/--missing-only/--stale/--dry-run/--concurrency/--prompt-version/--oversample/--no-challenger`，不追求机械每题 2 条（P0 核心题 3–4、普通题 1–2、低价值题可 0，上限 4），按 batch 落盘并写 provenance；`npm run question:validate-variants` 为发布门禁（阻断：stale/orphan/语义重复/语言质量/测量意图雷同/路径重复/kind 不符/format 不一致/重复 id/数量异常；审计：疑似同路径/推理链薄弱/难度驱动信号/coverage 矩阵/Concept×Angle×CognitiveTask）。**红线**：不建 Variant 专用 Agent、Runtime 不自动写回、类型锁死 4 种；runtime 不做质量治理（`variantPool.ts` 只做确定性选择）。
- **离线超采 + 质量质询（ADR-070，2026-09-08 经 ADR-078 升级为路径级，2026-09-15 经 ADR-082 加确定性 path 门禁）**：离线管线是四段式漏斗 —— `canonical → 超采 N×count 候选 → 确定性闸门（validateVariant + 语言门禁 + difficulty 驱动项 + 选项级 CJK-Dice 去重 + 幸存者间语义级去重）→ assessment 模式追加确定性 path 门禁（三段式声明 → 声明与 canonical 不高度相似（`isReasoningPathTooSimilar`：逐字相同或 target≥95 且 goal≥90 即拒）→ 双方 `inferAssessment` 题面推断 → 推断签名 Dice<82，任一失败即 pathreject；后者比较的是推断签名而非 LLM 声明）→ 质量质询 → 贪心 MMR 多样性排序取 top-N 落盘`。assessment 模式只允许 `surface-options | context-options`（不改选项不可能换 path）。
...
- **fuzzball 的中文相似度不可信（变体门禁已弃用）**：fuzzball 的 `token_set_ratio` 依赖词边界切分，
  ...
  又把「选项逐字相同、只换题干」的变体对误判为高相似（variant-vs-variant near-dup 漏检）。
  **变体三处门禁已改用 `src/domain/textSimilarity.ts` 的 CJK 感知字符级 Dice**（`cjkDice`：中文按单字、
  拉丁按词切，再算 token 多重集 Dice）。校准（temp/probe-realpool.mjs / probe-optonly.mjs）：长中文选项下
