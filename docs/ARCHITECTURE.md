...
- **离线超采 + 质量质询（ADR-070，2026-09-08 经 ADR-078 升级为路径级，2026-09-15 经 ADR-082 加确定性 path 门禁）**：离线管线是四段式漏斗 —— `canonical → 超采 N×count 候选 → 确定性闸门（validateVariant + 语言门禁 + difficulty 驱动项 + 选项级 CJK-Dice 去重 + 幸存者间语义级去重）→ assessment 模式追加确定性 path 门禁（三段式声明 → 与 canonical 显式 assessment 不同 → 双方 `inferAssessment` 题面推断 → 推断签名 Dice<82，任一失败即 pathreject；比较的是推断签名而非 LLM 声明）→ 质量质询 → 贪心 MMR 多样性排序取 top-N 落盘`。assessment 模式只允许 `surface-options | context-options`（不改选项不可能换 path）。
