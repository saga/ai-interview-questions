 ...
 *
 * 设计权衡（trade-off）：
 * - 「概念优先」与「topic 优先」双轨：优先沿知识图谱概念面选「最该验证的概念」，让探题更贴近真实薄弱点；
 *   但若知识节点未挂 concepts（无概念面），或概念选中后无对应题库题且 allowProbe=false，则退化回原 topic/angle 逻辑，
 *   保证「永远出得出题」而非卡死——向后兼容优先于特性。
 * - 最终兜底采用「证据最少优先」(angleEvidence 升序)：越没练过的 (topic,angle) 越先被问，复用 Learner 的探针语义，
 *   避免机械按难度顺序导致「总在问同一类」，提升覆盖效率。
