...
/**
 * 计算某道题在本次会话中「允许呈现」的形态集合（题目自带形态 ∩ 全局允许形态）。
 *
 * 设计要点：
 * - allowedFormats 为 undefined 或空数组 → 视为「不限」，返回题目自带的全部形态（choice/open）。
 *   这样上层（buildSession / planComposition / effectiveFormats）可以用「不传」表达「全部放行」，
 *   不必到处手写 ['choice','open']，并与 InterviewDefinition.formats 的「空=不限」约定对齐。
 * - 形如 ['choice'] 只放行选择题；['open'] 只放行开放题。
 * - 返回顺序固定为 [choice?, open?]：选择题永远先于开放题，便于上层「默认优先 choice」的取法。
 *
 * 为什么不做「按比例随机」：形态分配是确定性逻辑，随机配比交由 planComposition 在组卷阶段统一处理；
 * 本函数只回答「能不能出」，把「怎么分配」留到上层，避免职责混乱。
 */
 ...
 *   只有 open 形态的题保持 open——超额时优先与候选池未抽中的双形态题原位换题，无题可 换则裁掉；
 * - 整个候选池只有一种可用形态时跳过配比。
 *
 * 设计权衡（trade-off）：
 * - 为什么「从尾部翻 open 而非头部」：rawPicked 头部通常是 priority 主题（薄弱主题优先），把它们保留为
 *   choice 能更快出题、更快出分、降低 LLM 负担；尾部多为补充题，翻成 open 对核心覆盖影响最小。
 * - 为什么「超额先裁」而非强行保留：宁可少一题，也比塞入一道脱离配额占比的开放题更可控；
 *   开放题依赖 LLM 评分，占比失控会拖慢整场并放大评分失败概率。
 * - 为何选「固定配额」而非「完全随机」：稳定可预期的用户体验（约 7:3），避免相邻两次组卷体验差异过大。
 */
