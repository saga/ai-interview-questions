...
/** assessment contract：决定"这道题测什么"的最小字段集合。 */
...
export function assessmentContractOf(q: Pick<Question, 'topic' | 'angle' | 'difficulty'>): AssessmentContract {
  return { topic: q.topic, angle: q.angle, difficulty: q.difficulty };
}
...
export function isAssessmentChange(a: AssessmentContract, b: AssessmentContract): boolean {
  return a.topic !== b.topic || a.angle !== b.angle || a.difficulty !== b.difficulty;
}
