...
describe('pickNextAdaptive', () => {
  ...
  it('首轮（无信号）返回 move-on 的题且不依赖上一主题', () => {
    const r = pickNextAdaptive(POOL, [], conceptGraph, undefined, rngSeq([0]));
  ...
  it('broaden：答得好 → 从相关主题选题，不重复同主题', () => {
    ...
    const r = pickNextAdaptive(POOL, [sig('tool-calling', 95, 'medium')], conceptGraph, undefined, rngSeq([0, 0]));
    expect(r!.strategy).toBe('broaden');
    expect(r!.question.topic).toBe('mcp');
  });

  it('deep-dive：中等表现 → 同主题更高难度优先', () => {
    const r = pickNextAdaptive(POOL.filter((x) => x.id !== 'mcp-1'), [sig('tool-calling', 70, 'medium')], conceptGraph, undefined, rngSeq([0]));
  ...
  it('gap-probe：答得差 → 先降难度，再退前置主题', () => {
    ...
    const r1 = pickNextAdaptive(POOL, [sig('tool-calling', 30, 'hard')], conceptGraph, undefined, rngSeq([0]));
  ...
  it('已问过的题不会再次出现（由调用方过滤题池保证）', () => {
    ...
    const r = pickNextAdaptive(pool, [sig('tool-calling', 95, 'medium')], conceptGraph, undefined, rngSeq([0, 0, 0]));
    expect(Object.keys(asked)).not.toContain(r!.question.id);
  });

  it('空池返回 null', () => {
    expect(pickNextAdaptive([], [], conceptGraph)).toBeNull();
  });
});
