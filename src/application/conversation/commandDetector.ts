 ...
 * 「继续 / 换题 / 跳过」类命令。**必须整句匹配**。
 *
 * 旧实现只有 `^` 前缀、没有 `$`，于是「继续解释一下」「继续讲刚才那个」这类
 * **接着讲**的请求也被判成「要下一题」——与 Copilot「解释考点、拆解思路」的定位直接冲突：
 * 用户想让它接着讲，结果题目被换掉了。
 *
 * 末尾允许三类尾巴（它们是同一动作的自然说法，不是新意图）：
 *   ① 指代填充：这道题 / 这题 / 这个 / 一题 / 一个（「跳过这道题」）
 *   ② 难度修饰：难一点 / 更难 / 简单一点 / 容易一点（「下一题难一点」）
 *   ③ 语气词：吧
 */
const CONTINUE_PATTERN =
  /^(继续|下一题|下一道|再来一道|再来一个|继续考|再出一题|再出一道|下一个|继续面试|追问|针对.*(继续|追问)|再难一点|难一点|简单一点|容易一点|换一道|换一题|换题|换一个题|跳过|不想答|不答了)(这题|这道题|这个|一题|一个)?(难一点|更难|简单一点|容易一点)?(吧)?$/u;
...
/**
 * 「接着讲」类续问：以「继续/接着」开头并紧跟说明动词（继续解释一下 / 继续讲刚才那个）。
 *
 * 必须与 CONTINUE_PATTERN 配对使用：前者把「继续 + 说明动词」排除出命令，
 * 后者保证它落到 Copilot 而不是答案通道——开放题下任何非空文本都会被当成一次作答，
 * 没有这条，用户「让它接着讲」的话会被当成答案去评分。
 */
const CONTINUE_EXPLAIN_PATTERN = /^(继续|接着)(讲|说|解释|解读|展开|聊|介绍|分析|补充|举例)/u;
...
export function isHelpSeeking(input: string): boolean {
  ...
  // 「继续解释一下」：接着讲，不是要下一题（见 CONTINUE_EXPLAIN_PATTERN）。
  if (CONTINUE_EXPLAIN_PATTERN.test(text)) return true;
