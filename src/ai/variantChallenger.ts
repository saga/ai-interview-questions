...
//
// 本模块是产出后的**质量漏斗**：对候选变体逐条打 6 个维度的分，供超采—筛选取 top-N。
// 红线：**只在离线 pipeline 跑**，不进 runtime、不做在线双模型——runtime 仍是
// one-shot + fallback 原题（ADR-069）。

export const VARIANT_CHALLENGER_SYSTEM = `[PROMPT-VERSION v2]
...
6. assessment-identity：变体仍在测量**同一 Knowledge 下的判断**——未引入新的隐含知识前提（答对不需要原题之外的知识），题目的真正难点没有移位（难在同一处，只是换了一条推理路径去够它）。若变体自声明了新的 angle / cognitiveTask / assessment，本维度同时检查该声明是否名副其实（声明换路径却只是换措辞 → fail）。
...
只输出一个 JSON 对象，不要 Markdown 或额外文字。字段：
{
  "dimensions": [
    ...
    {"dimension": "assessment-identity", "pass": true, "note": "..."}
  ],
  "summary": "一句话结论"
}

判定尺度：宁严勿松。answer-preserved 只要有一项真假翻转就必须 pass=false。
六个维度全部 pass 才算合格变体；任一维度 pass=false 即整条候选不合格。`;
...
/** 变体的题干/选项表示（generateVariant 的返回形状），可附带自声明的测量面。 */
export interface VariantShape {
  question: string;
  options?: string[];
}

/** 质询输入：表达 + 可选的自声明测量面（assessment-mode 候选有，presentation 候选无）。 */
export type ChallengeableVariant = VariantShape & VariantMeasurementFace;

function renderFace(face: VariantMeasurementFace): string {
  const parts: string[] = [];
  if (face.angle) parts.push(`angle=${face.angle}`);
  if (face.cognitiveTask) parts.push(`cognitiveTask=${face.cognitiveTask}`);
  if (face.assessment) parts.push(`target=${face.assessment.target}；reasoningGoal=${face.assessment.reasoningGoal}`);
...
export function buildVariantChallengeUser(
  canonical: Question,
  variant: ChallengeableVariant,
  format: 'choice' | 'open',
): string {
...
原题测量面：angle=${canonical.angle}；cognitiveTask=${canonical.cognitiveTask ?? '（未声明）'}
原题测量意图：${canonical.assessment ? `target=${canonical.assessment.target}；reasoningGoal=${canonical.assessment.reasoningGoal}` : '（未声明）'}

【变体】（呈现形态：${format}；答案索引、解析、topic、difficulty 均由程序继承自原题，不需检查）
题干：
${variant.question}
...
export function parseVariantChallenge(raw: string): VariantChallenge {
  ...
  // 缺维度 = 旧 prompt / 截断输出：缺席的维度按 fail 计（宁严勿松），且 score 分母
  // 恒为全维度数——否则「少答一维」反而能拿高分。
  const seen = new Set(parsed.data.dimensions.map((d) => d.dimension));
  const missing = VARIANT_CHALLENGE_DIMENSIONS.filter((d) => !seen.has(d));
  const allFailed = [...failed, ...missing];
  const passCount = VARIANT_CHALLENGE_DIMENSIONS.length - allFailed.length;
  return {
    ok: allFailed.length === 0,
    score: passCount / VARIANT_CHALLENGE_DIMENSIONS.length,
    failed: allFailed,
    summary:
      missing.length > 0 ? `质询输出缺维度（${missing.join('/')} 按 fail 计）：${parsed.data.summary}` : parsed.data.summary,
  };
}
